...
if [ "$count" -eq 1 ]; then
   ...
   if [ -f "$IP_VALUE_FILE" ]; then
      ...
      echo "default router ip is 192.168.27.1" >> $LOGFILE
   fi
...
if command -v dockerd >/dev/null 2>&1; then
   echo "Detected Docker，configuring Firewall Rule..."
...
EOF
   ...
   echo "Undetected Docker，Skipping the configure of firewall rule."
fi
...
NEW_DESCRIPTION="Compiled by Jaguar AI Studio"
